...
impl HarnessDriver for OpencodeDriver {
    ...
    fn install_hint(&self) -> Option<&str> {
        Some("Install OpenCode from https://opencode.ai")
    }

    fn detect(&self) -> bool {
        app_exists(&["/Applications/OpenCode.app"]) && which("opencode").is_some()
    }

    fn probe(&self, _config: &HarnessInstanceConfig) -> HarnessProbeResult {
        let detected = self.detect();
        let version = if detected {
            run_version_probe("opencode", &["--version"])
        } else {
            None
        };
        HarnessProbeResult {
    ...
    fn run_install_agent(
        ...
        log.push_str("Running opencode for MCP analysis…\n");
        ...
        match output {
            ...
            Err(e) => {
                log.push_str(&format!("opencode failed: {e}\n"));
            }
        }
